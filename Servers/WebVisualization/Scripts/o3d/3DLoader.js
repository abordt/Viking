...
function updateCamera() {
    ...
    g_lightPosParam.value = [0, 200, -800];
}
...
function loadFile(context, path) {
    function callback(pack, parent, exception) {
        ...
        if (exception) {
            ...
            if (info === "problem")
                document.getElementById("modelMessage").innerHTML = "Could not generate Model, try again or try unchecking the update box to load the stored model";
            else
                 document.getElementById("modelMessage").innerHTML = "Loading failed. Please try again";   
            ...
            for (var m = 0; m < materials.length; ++m) {
                ...
                msg.innerHTML = " <b>Executing...</b>";
                document.getElementById("message").innerHTML = " <b>Enter / Choose a Valid Cell ID</b> ";

                var prefix = "";

                if (sampleFirst) {
                    prefix = "First time Sample ";
                }

                document.getElementById("modelMessage").innerHTML = prefix + "Model Generated! use mouse to explore";
            ...
            if (document.getElementById("freshQuery").checked)
                document.getElementById("freshQuery").checked = false;
...
function initStep2(clientElements) {
    ...
    path = 'http://connectomes.utah.edu/ColladaFiles/MarcLab(connectome.utah.edu)_Rabbit(MarcLab)_5575.dae';
...
function doload() {
    ...
    param += document.getElementById("ctl00_MainContent_labName").options[document.getElementById("ctl00_MainContent_labName").selectedIndex].text.replace(/^\s+|\s+$/g, '') + ",";
    param += document.getElementById("ctl00_MainContent_dataSource").options[document.getElementById("ctl00_MainContent_dataSource").selectedIndex].text.replace(/^\s+|\s+$/g, '') + ",";

    var ids = "180";

    if (document.getElementById("cellID").disabled)
        ids = document.getElementById("ctl00_MainContent_structureID").value+ ",";
    else
        ids = document.getElementById("cellID").value + ",";
    ...
    param += document.getElementById("hops").value + ",";
...
function uploadAction(filePath) {
    ...
    var urlAppend = "VikingPlot/GetObjectFile?query=";
    ...
    document.getElementById("modelMessage").innerHTML = "Generating/Searching 3D model(s) of the requested cell(s) on server, this may take a while...";
...
function FileWritten() {
    ...
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
        ...
        if (xmlHttp.responseText == "Not found") {
            ...
            document.getElementById("modelMessage").innerHTML = "Successful...now Downloading 3D model into browser...";

            var re = new RegExp('.*/(.*?)', 'im');
            var mat = info.toString().match(re)[0];

            var name = info.replace(mat, "");


            var re = new RegExp('.*_(.*?)', 'im');
            var mat = name.toString().match(re)[0];

            document.getElementById("3dId").innerHTML = " " + name.replace(mat).replace(".json", "").replace('undefined', '');
            var parts = name.replace(".json", "").split('_');
            document.getElementById("3dLab").innerHTML = parts[0]
            document.getElementById("3dDb").innerHTML = parts[1];

            document.getElementById("3dLink").innerHTML = name.replace(".json", ".dae");
            document.getElementById("3dLink").href = info.replace(".json", ".dae");
